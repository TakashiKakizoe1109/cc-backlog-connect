...
2. **設定ファイルが存在しない場合**（初回セットアップ）:
   ...
   - **モード**: `read`（読み取りのみ、デフォルト）または `write`（書き込み許可）。
     安全のためデフォルトは `read`。課題の作成・更新・削除を行う場合は `write` を選択。

3. **設定ファイルが存在する場合**:
   現在の設定内容を表示してください（API キーは先頭4文字と末尾4文字以外をマスク）。
   mode も表示されます。ユーザーが変更したい場合は、変更したい値だけ質問してください。
