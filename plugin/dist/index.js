...
async function main() {
    ...
    switch (command) {
        ...
        case "sync": {
            ...
            const resolveNameIds = (nameVal, type) => {
                if (!nameVal || typeof nameVal !== "string")
                    return undefined;
                return nameVal.split(",").map((n) => {
                    const id = (0, metadata_1.resolveNameToId)(type, n.trim());
                    if (id === undefined) {
                        console.error(`Error: Cannot resolve "${n.trim()}" for ${type}. Run "cc-backlog project-info ${type}" first.`);
                        process.exit(1);
                    }
                    return id;
                });
...
main()
    ...
    .catch((err) => {
    if (exiting)
        return; // process.exit(0) was called; suppress secondary error
